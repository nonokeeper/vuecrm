<template>
    <main>
      ...
      <div v-if="loaded && authorized" class="grid grid-cols-4 gap-4">
        <div class="ml-4 col-span-2">Showing <span v-if="nbTotalData > 0">{{ beginCursor }} to {{ endCursor }} of </span>{{ nbTotalData }} results</div>
        <div>
        ...
        <div class="text-right mr-4">
          ...
          <input class="dark:text-black w-16" type="number" v-model="pageNumber" @change="refresh()"/>
           / {{ pageTotal }}&nbsp;
      ...
      <data-create v-if="create" @cancelCreate="cancelCreate" @created="created" :data-meta="dataMeta"/>
      -->
    </main>
  ...
  */
  ...
  const entity = useRouter().currentRoute.value.query.entity;
  ...
  onBeforeMount( async () => {
    ...
    //if (token != '') {
      ...
      if (nbTotalData.value >= 0) {authorized.value = true}
    //}
  ...
  const filterData = async (meta:string, operator:string, val:string) => {
    ...
    if (token != '') {
      dataFiltered.value = await DataService.getData(FIRSTPAGE, size.value, metaFilter.value, operatorFilter.value, valFilter.value, token);
